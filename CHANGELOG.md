...
### 2026-09-05 — 新增站点图标（favicon）

**变更类型**：新增

**说明**：项目此前没有 favicon（浏览器 404、标签页无图标）。新增 `src/app/icon.png`（Next.js App Router 约定，自动注入 favicon link）——紫色渐变圆角瓦片 + 与对话页品牌头像同款的白色火花图形，320×320。设计源文件 `docs/images/icon.html`，改动后在浏览器 320×320 截图即可重新导出。

**变更文件**：

| 文件 | 操作 | 说明 |
|------|------|------|
| `src/app/icon.png` | 新增 | 站点图标（构建时自动转 favicon） |
| `docs/images/icon.html` | 新增 | 图标设计源文件 |

**影响范围**：前端

---

### 2026-09-05 — 新增 API 接口文档

**变更类型**：新增

**说明**：新增 `docs/API.md`，固化三个接口（/api/chat、/api/dashboard、/api/hot-questions）的请求 / 响应契约、history 校验规则、限流行为（含"非法请求也计入额度"的设计）、错误码表与 SSE 流格式，并同步到两版 README 的项目结构图。

**变更文件**：

| 文件 | 操作 | 说明 |
|------|------|------|
| `docs/API.md` | 新增 | API 接口文档 |
| `README.md` / `README.en.md` | 修改 | 项目结构补 API.md |
